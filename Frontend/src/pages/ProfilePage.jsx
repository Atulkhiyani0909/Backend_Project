...
export default function ProfilePage() {
  ...
  useEffect(()=>{
    async function getUserDetails() {
      let data =await axios.get('http://localhost:8080/api/v1/users/getUserChannel/@johndoe')
      ...
      <div className="max-w-7xl mx-auto px-4 pb-8">
        <div className="flex flex-col md:flex-row items-start gap-6 -mt-12 relative">
          ...
          <div className="flex-1 mt-4 md:mt-0">
            <h1 className="text-3xl font-bold dark:text-white">{userProfile.username}</h1>
            
            <div className="flex flex-wrap gap-6 mt-4 text-gray-600 dark:text-gray-400">
              <div className="flex items-center gap-2">
                <Users className="w-5 h-5" />
                <span>{userProfile.subscriberCount} subscribers</span>
              </div>
              <div className="flex items-center gap-2">
                <Eye className="w-5 h-5" />
                <span>{userProfile.subscriptionCount} total views</span>
              </div>
              <div className="flex items-center gap-2">
